...
FEATURES_DIR = "./data/mfccfeatures"
BATCH_SIZE = 32
EPOCHS = 30
LEARNING_RATE = 0.001
...
def evaluate_testing_holdout(model, test_loader, X_test, y_test):
    ...
    with torch.no_grad():
        for batch_X, batch_y in test_loader:
            ...
            # Use .view(-1) to flatten the tensor layout explicitly on the CPU
            all_predictions.append(predicted.cpu().view(-1))
            all_true_labels.append(batch_y.view(-1))
            
    # Concatenate the collected tensor blocks securely before converting to numpy
